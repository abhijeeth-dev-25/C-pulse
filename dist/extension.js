...
class CPulsePanel {
    ...
    _watchWebviewBuild(extensionUri) {
        const jsOutPath = vscode_1.Uri.joinPath(extensionUri, "dist", "webview", "assets", "index.js").fsPath;
        // Use VS Code's built-in file watcher
        const watcher = vscode_1.workspace.createFileSystemWatcher(`**${path.sep}dist${path.sep}webview${path.sep}assets${path.sep}*.js`);
        const reload = () => {
            if (CPulsePanel.currentPanel) {
    ...
    _getWebviewContent(webview) {
        ...
        const stylesUri = webview.asWebviewUri(vscode_1.Uri.joinPath(this._extensionUri, "dist", "webview", "assets", "index.css"));
        // The JS file from the React build output
        const scriptUri = webview.asWebviewUri(vscode_1.Uri.joinPath(this._extensionUri, "dist", "webview", "assets", "index.js"));
